...
class BaseColorizationDataset(Dataset):
    ...
    def __init__(
        ...
        # Сохраняем crop_size как атрибут класса
        self.crop_size = None
...
def create_transform(
    ...
    if resize_mode == 'resize':
        ...
        # Используем локальную переменную crop_size, а не атрибут класса
        local_crop_size = crop_size or img_size
        transforms.append(T.CenterCrop(local_crop_size))
        if local_crop_size != img_size:
            transforms.append(T.Resize((img_size, img_size), interpolation=T.InterpolationMode.BICUBIC))
    else:
...
def create_paired_transform(
    ...
    def paired_transform(grayscale_img: Image.Image, color_img: Image.Image) -> Tuple[Image.Image, Image.Image]:
        ...
        if resize_mode == 'resize':
            ...
            if random_crop:
                # Получаем параметры для случайного кропа
                i, j, h, w = T.RandomCrop.get_params(grayscale_img, output_size=(local_crop_size, local_crop_size))
                grayscale_img = TF.crop(grayscale_img, i, j, h, w)
                color_img = TF.crop(color_img, i, j, h, w)
            else:
                # Применяем центральный кроп
                grayscale_img = TF.center_crop(grayscale_img, local_crop_size)
                color_img = TF.center_crop(color_img, local_crop_size)
