...
```
> [!NOTE]
> Реализация использует выход ConvNeXt‑Tiny (`F1`, масштаб H/4) как вход для CoAtNet‑light и выбирает более глубокую стадию выхода CoAtNet. Это даёт фактические масштабы признаков `F2 ≈ H/32` и `F3 ≈ H/64` при текущей конфигурации, что обеспечивает более сильный глобальный контекст. При необходимости можно переключить стадию CoAtNet на более раннюю и приблизиться к `H/8` и `H/16`, но это потребует перенастройки каналов и последующего переобучения.
