...
Датасеты `AdvancedColorizationDataset` и `SimpleColorizationDataset` читают КРАСОЧНЫЕ изображения из указанных папок РЕКУРСИВНО (подпапки допускаются). Парные копии (серые/деградированные) НЕ требуются — L‑канал формируется на лету из RGB.

```
├── train/                 # цветные изображения для обучения (рекурсивно)
│   ├── class_a/0001.jpg
│   └── landscapes/img_a.png
└── val/                   # цветные изображения для валидации (рекурсивно)
    └── ...
```

Пути по умолчанию задаются в `configs/default.yaml` → `paths`:
- `paths.train_dir: data/train`
- `paths.val_dir: data/val`
- при наличии `paths.data_root` относительные пути берутся от него.
...
- Размеры исходников произвольные. На обучении и валидации кадры приводятся к квадрату `training.image_size` (по умолчанию `256`, рекомендуется кратность 32; 8 допустимо). На инференсе паддинг до кратности по умолчанию 32 и настраивается через `inference.pad_divisor` или CLI `--pad-div`. Правила геометрии берутся из конфига:
...
- `validation`: `enabled`, `batch_size`, `ssim_window`, `dists`
- `logging`: TensorBoard/W&B
- `checkpointing`: политика и имена `latest`/`best`

Дополнительно: краткий обзор новых опций (DLB entropy-aware, GAN label smoothing/R1, CIEDE2000, TTA/tiling) смотрите в [CONFIGURATION.md](CONFIGURATION.md).
...
- DISTS — целевой ориентир ≤ 0.20 (требуется пакет `DISTS-pytorch` и `validation.dists: true`)


### Анализ по категориям
Подготовьте собственные списки файлов/подкаталоги и запускайте инференс по группам для сравнения метрик SSIM/DISTS.
