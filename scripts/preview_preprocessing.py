...
def apply_pipeline(img: Image.Image,
    ...
    L_gray_before = None
    if (mode == "train" and dataset_kind == "advanced" and enable_defects 
        and isinstance(defects_cfg, dict) and bool(defects_cfg.get("enabled", False))):
    ...
    if (not disable_jitter) and ab_jitter > 0 and mode == "train" and dataset_kind == "advanced":
...
def _make_comparison(original: Image.Image, processed: Image.Image) -> Image.Image:
    ...
    draw.text((o.width + 15, 5), f"proc {processed.size[0]}x{processed.size[1]}", fill=(255,255,0))
    return canvas
...
def main():
    parser = argparse.ArgumentParser(description="Предпросмотр пайплайна подготовки изображений без обучения")
    parser.add_argument("--input", type=str, required=True, help="Путь к изображению (одному файлу)")
    ...
    parser.add_argument("--dataset", type=str, choices=["advanced", "simple"], default=None,
                        help="Перекрыть training.dataset из конфига")
    parser.add_argument("--output", type=str, default=None, help="Папка для сохранения превью")
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--disable_flip", action="store_true")
    parser.add_argument("--disable_jitter", action="store_true")
    parser.add_argument("--save_L", action="store_true", help="Сохранить L-канал (ЧБ) как <stem>_L_gray.png для проверки входа модели")
    parser.add_argument("--enable_defects", action="store_true", help="Применить дефекты L-канала из training.aug.defects конфига к предпросмотру (train/advanced)")
    ...
    ds_kind = args.dataset if args.dataset is not None else train_cfg.get("dataset", "advanced")
    ...
    out_dir = Path(args.output) if args.output else Path(cfg.get("paths", {}).get("experiments", "experiments/exp_default")) / "preview_preprocess"
    ...
    stem = inp.stem
    o, g, l, lgray = save_triplet(out_dir, stem, result, save_L=args.save_L)

    print("Готово. Сохранены файлы:")
    print(f" - Оригинал:          {o}")
    print(f" - После геометрии:   {g}")
    print(f" - После Lab+jitter:  {l}")
