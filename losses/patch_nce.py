...
class PatchNCELoss(nn.Module):
    """
    Основной класс PatchNCE Loss для контрастного и градиентного обучения.
    
    Этот класс реализует главную функцию потерь для колоризатора согласно ТЗ:
    - Контрастное обучение помогает модели различать похожие участки
    - Градиентное обучение стабилизирует процесс обучения и сохраняет детали
    - Работа с патчами сохраняет мелкие текстуры и локальные особенности
    
    Args:
        num_patches (int): Количество патчей для выборки из каждой карты признаков
        patch_size (int): Размер патчей (обычно 1 для точечного сравнения)
        temperature (float): Температура для softmax в контрастном обучении
        nce_weight (float): Вес контрастной потери
        gradient_weight (float): Вес градиентной потери
        use_cosine_similarity (bool): Использовать косинусное сходство вместо скалярного произведения
        normalize_features (bool): Нормализовать ли признаки перед вычислением потерь
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Критерий для кросс-энтропии
        self.cross_entropy_loss = nn.CrossEntropyLoss(reduction='none')
        
        # Для подсчета статистики
        self.register_buffer('total_samples', torch.tensor(0.0))
        self.register_buffer('positive_similarities', torch.tensor(0.0))
        
    def extract_patches(self, features, indices=None):
        """
        Извлекает патчи из карты признаков.
        
        Args:
            features (torch.Tensor): Карта признаков [B, C, H, W]
            indices (torch.Tensor, optional): Предварительно выбранные индексы патчей
            
        Returns:
            torch.Tensor: Извлеченные патчи [B, num_patches, C]
            torch.Tensor: Индексы выбранных патчей [B, num_patches]
        """
        B, C, H, W = features.shape
        
        # Перестраиваем признаки в формат [B, C, H*W]
        features_flat = features.view(B, C, H * W)
        
        if indices is None:
            # Случайная выборка патчей
            num_available = H * W
            num_to_select = min(self.num_patches, num_available)
            
            indices = torch.randperm(num_available, device=features.device)[:num_to_select]
            indices = indices.unsqueeze(0).expand(B, -1)
        
        # Извлекаем патчи по индексам
        patches = torch.gather(
            features_flat, 
            2, 
            indices.unsqueeze(1).expand(-1, C, -1)
        )  # [B, C, num_patches]
        
        # Перестановка для получения [B, num_patches, C]
        patches = patches.transpose(1, 2)
        
        return patches, indices
    
    def compute_similarity_matrix(self, patches_a, patches_b):
        """
        Вычисляет матрицу сходства между двумя наборами патчей.
        
        Args:
            patches_a (torch.Tensor): Первый набор патчей [B, N, C]
            patches_b (torch.Tensor): Второй набор патчей [B, N, C]
            
        Returns:
            torch.Tensor: Матрица сходства [B, N, N]
        """
        ...
        if self.use_cosine_similarity:
            # Косинусное сходство уже включено в нормализацию
            similarity = torch.bmm(patches_a, patches_b.transpose(1, 2))
        else:
    ...
    def contrastive_loss(self, anchor_patches, positive_patches):
        """
        Вычисляет контрастную потерю между якорными и положительными патчами.
        
        Args:
            anchor_patches (torch.Tensor): Якорные патчи [B, N, C]
            positive_patches (torch.Tensor): Положительные патчи [B, N, C]
            
        Returns:
            torch.Tensor: Контрастная потеря
        """
        B, N, C = anchor_patches.shape
        
        # Вычисляем матрицу сходства
        similarity_matrix = self.compute_similarity_matrix(anchor_patches, positive_patches)
        
        # Применяем температурное масштабирование
        logits = similarity_matrix / self.temperature
        
        # Создаем метки - диагональные элементы являются положительными парами
        labels = torch.arange(N, device=logits.device).expand(B, N)
        
        # Вычисляем потерю кросс-энтропии
        loss = self.cross_entropy_loss(logits.view(-1, N), labels.view(-1))
        
        # Возвращаем среднюю потерю
        return loss.mean()
    
    def gradient_consistency_loss(self, features_a, features_b):
        """
        Вычисляет потерю согласованности градиентов между двумя картами признаков.
        
        Args:
            features_a (torch.Tensor): Первая карта признаков [B, C, H, W]
            features_b (torch.Tensor): Вторая карта признаков [B, C, H, W]
            
        Returns:
            torch.Tensor: Потеря согласованности градиентов
        """
        ...
        # Вычисляем градиенты для обеих карт признаков
        grad_a_x, grad_a_y = compute_gradients(features_a)
        grad_b_x, grad_b_y = compute_gradients(features_b)
        
        # Вычисляем L1 потерю между градиентами
        loss_x = F.l1_loss(grad_a_x, grad_b_x)
        loss_y = F.l1_loss(grad_a_y, grad_b_y)
        
        return loss_x + loss_y
    
    def update_statistics(self, anchor_patches, positive_patches):
        """
        Обновляет статистику для мониторинга обучения.
        ...
        """
        with torch.no_grad():
            # Вычисляем диагональные элементы матрицы сходства (соответствующие пары)
            similarity_matrix = self.compute_similarity_matrix(anchor_patches, positive_patches)
            diagonal_similarities = torch.diagonal(similarity_matrix, dim1=1, dim2=2)
            
            # Обновляем статистику
            batch_samples = diagonal_similarities.numel()
            batch_positive_sim = diagonal_similarities.sum()
            
            self.total_samples += batch_samples
            self.positive_similarities += batch_positive_sim
    
    def get_average_positive_similarity(self):
        """
        Возвращает среднее сходство положительных пар с момента последнего сброса.
        
        Returns:
            float: Среднее сходство положительных пар
        """
        if self.total_samples > 0:
            avg_sim = (self.positive_similarities / self.total_samples).item()
        else:
            avg_sim = 0.0
        
        # Сбрасываем статистику
        self.total_samples.zero_()
        self.positive_similarities.zero_()
    ...
    def forward(self, encoder_features, decoder_features):
        ...
        decoder_patches, _ = self.extract_patches(decoder_features, patch_indices)
        
        # Вычисляем контрастную потерю
        nce_loss = self.contrastive_loss(encoder_patches, decoder_patches)
        
        # Вычисляем градиентную потерю
        gradient_loss = self.gradient_consistency_loss(encoder_features, decoder_features)
        
        # Общая потеря
        total_loss = (self.nce_weight * nce_loss + 
                     self.gradient_weight * gradient_loss)
        
        # Обновляем статистику
        self.update_statistics(encoder_patches, decoder_patches)
...
# Функция для создания PatchNCELoss с параметрами по умолчанию
def create_patch_nce_loss(config=None):
    """
    Создает основную функцию потерь PatchNCELoss с параметрами по умолчанию или пользовательской конфигурацией.
    
    Args:
        config (dict, optional): Словарь с параметрами конфигурации
        
    Returns:
        PatchNCELoss: Основная функция потерь согласно ТЗ
    """
    # Параметры по умолчанию для основного класса PatchNCELoss
    default_config = {
        'num_patches': 256,
        'patch_size': 1,
        'temperature': 0.07,
    ...
    # Создаем основную функцию потерь с указанными параметрами
    loss_fn = PatchNCELoss(**default_config)
    
    return loss_fn


# Функция для создания MultiScalePatchwiseLoss с параметрами по умолчанию
def create_multiscale_patch_nce_loss(config=None):
    """
    Создает многомасштабную функцию потерь MultiScalePatchwiseLoss с параметрами по умолчанию или пользовательской конфигурацией.
    
    Args:
        config (dict, optional): Словарь с параметрами конфигурации
        
    Returns:
        MultiScalePatchwiseLoss: Многомасштабная функция потерь
    """
    # Параметры по умолчанию для многомасштабной версии
    default_config = {
        'feature_dims': [256, 512, 1024],
        'patch_sizes': [1, 3, 5],
        'patch_counts': [256, 128, 64],
    ...
    # Создаем многомасштабную функцию потерь с указанными параметрами
...
if __name__ == "__main__":
    # Пример использования основного класса PatchNCELoss
    print("=== Тестирование основного класса PatchNCELoss ===")
    
    # Создаем основную функцию потерь
    main_loss_fn = create_patch_nce_loss()
    ...
    # Вычисляем потери
    main_loss_result = main_loss_fn(encoder_features, decoder_features)
    
    # Выводим результаты
    print("Результаты основного класса PatchNCELoss:")
    for key, value in main_loss_result.items():
        if isinstance(value, torch.Tensor):
            print(f"  {key}: {value.item():.4f}")
        else:
            print(f"  {key}: {value}")
    
    print("\n=== Тестирование многомасштабного класса MultiScalePatchwiseLoss ===")
    
    # Создаем многомасштабную функцию потерь
    multiscale_loss_fn = create_multiscale_patch_nce_loss()
    
    # Признаки из энкодера и декодера (многоуровневые)
    encoder_features_ms = [
    ...
    # Вычисляем потери
    features = {
        'encoder': encoder_features_ms,
        'decoder': decoder_features_ms
    }
    
    multiscale_loss_result = multiscale_loss_fn(features, predicted, target)
    
    # Выводим результаты
    print("Результаты многомасштабного класса MultiScalePatchwiseLoss:")
    for key, value in multiscale_loss_result.items():
        if isinstance(value, torch.Tensor):
            print(f"  {key}: {value.item():.4f}")
    ...
    print("\n✅ Класс PatchNCELoss успешно добавлен согласно ТЗ!")
    print("🎯 Реализованы контрастное и градиентное обучение для колоризации")
    print("🔧 Сохранена вся существующая функциональность")