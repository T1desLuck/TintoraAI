...
class PhotometricSmoothnessLoss(nn.Module):
    ...
    def forward(self, L: torch.Tensor, a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
        L01 = (L + 1.0) * 0.5
        Lx, Ly = _sobel(L01)
        ax, ay = _sobel(a)
        bx, by = _sobel(b)
        ...
        tv = (wx * torch.abs(ax) + wy * torch.abs(ay) + wx * torch.abs(bx) + wy * torch.abs(by)).mean()
        return tv
...
class OMMClusterLoss(nn.Module):
    ...
    def forward(self, F2n: torch.Tensor, mem_map: torch.Tensor) -> torch.Tensor:
        ...
        B, C, h2, w2 = F2n.shape
        mem_r = F.interpolate(mem_map, size=(h2, w2), mode="bilinear", align_corners=False)
