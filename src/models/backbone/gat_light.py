...
class GATLight(nn.Module):
    ...
    def __init__(self, in_channels=192, out_channels=384, img_size=224, patch_size=4, embed_dim=96, 
                 depths=[2, 2], num_heads=[3, 6], window_size=7, mlp_ratio=4., 
                 qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0., 
                 drop_path_rate=0.1, norm_layer=nn.LayerNorm, ape=False, patch_norm=True, **kwargs):
        super().__init__()
        # Сохраняем входное разрешение
        self.input_resolution = (img_size // (patch_size * 8), img_size // (patch_size * 8))
        self.down = nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=2, padding=1)
        self.blocks = nn.ModuleList([TransformerBlock(out_channels, num_heads=num_heads[0]) for _ in range(depths[0])])
