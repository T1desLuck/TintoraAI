...
class TintoraAI(nn.Module):
    ...
    def __init__(
        ...
        omm_config: Optional[Dict[str, Any]] = None,
        ...
        # Predeclare optionals for mypy
        self.guide: Optional[GuideNet]
        self.pool_f3: Optional[nn.AdaptiveAvgPool2d]

        # Стадии бэкбона
        self.stage1 = ConvNeXtTiny()
        self.stage2 = CoAtNetLight()
        self.stage3 = GATLight(in_channels=192, out_channels=384)  # type: ignore[call-arg]
        ...
        self.omm_proj = (
            nn.Conv2d(c2, self.omm_dim, kernel_size=1)
            if c2 != self.omm_dim
            else nn.Identity()
        )
        ...
        self.ctx_hidden = 256
        self.crb = ColorReasoningBlock(
            c3=c3,
            cmem=self.omm_dim,
            film_stage_dims=(c3, c2, c1),
            geom_ch=1 + 1 + 3,
            ctx_hidden=self.ctx_hidden,
        )
    ...
    def forward(
        ...
        omm_read_only: Optional[bool] = None,
        ...
        omm_out = self.omm(
            F2_for_omm, gt_ab=gt_ab, read_only=omm_read_only
        )  # учебный план может «замораживать» OMM
        ...
        if self.guide is not None:
            ...
            assert self.pool_f3 is not None
    ...
    def compute_normals(D: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
        ...
        # Ядра Собеля
        kx = torch.tensor(
            [[1, 0, -1], [2, 0, -2], [1, 0, -1]], dtype=D.dtype, device=D.device
        ).view(1, 1, 3, 3)
        ky = torch.tensor(
            [[1, 2, 1], [0, 0, 0], [-1, -2, -1]], dtype=D.dtype, device=D.device
        ).view(1, 1, 3, 3)
