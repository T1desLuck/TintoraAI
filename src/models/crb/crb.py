...
class FiLMGenerator(nn.Module):
    ...
    def __init__(self, in_dim: int, stage_dims: Tuple[int, ...] = (384, 192, 96), hidden: int = 256):
        super().__init__()
...
class ColorReasoningBlock(nn.Module):
    ...
    def __init__(self, c3: int, cmem: int, film_stage_dims=(384, 192, 96), geom_ch: int = 1 + 1 + 3, ctx_hidden: int = 256):
        super().__init__()
        ...
        self.film = FiLMGenerator(in_dim=ctx_hidden, stage_dims=film_stage_dims, hidden=ctx_hidden)
    ...
    def forward(
        ...
        mem_ds = F.interpolate(mem_map, size=(H, W), mode="bilinear", align_corners=False)
        geom = torch.cat([D, I, normals], dim=1)
        ...
        if guide_ctx is not None:
            ...
            if guide_ctx.shape[-1] == ctx.shape[-1]:
                ...
                raise ValueError(f"guide_ctx dim {guide_ctx.shape[-1]} != ctx_hidden {ctx.shape[-1]}")
