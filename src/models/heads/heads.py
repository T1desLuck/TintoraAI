...
class UpBlock(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, skip: torch.Tensor, out_size: Tuple[int, int]
    ) -> torch.Tensor:
...
class DepthHead(nn.Module):
    ...
    def forward(
        self, F2: torch.Tensor, F3: torch.Tensor, out_hw: Tuple[int, int]
    ) -> torch.Tensor:
        x3 = self.reduce3(F3)
        x2 = self.reduce2(F2)
...
class IlluminationHead(nn.Module):
    ...
    def forward(
        ...
        x2 = self.reduce2(F2)
        x = self.up(x3, x2, out_hw)
        illum_map = torch.sigmoid(self.out(x))
        return illum_map
