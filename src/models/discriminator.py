...
class PatchDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc: int = 3, ndf: int = 64, n_layers: int = 3, use_spectral_norm: bool = True):
        super().__init__()
        kw = 4
        padw = 1
        layers = []
        # первый слой
        layers += [
            spectral_norm_if(nn.Conv2d(input_nc, ndf, kernel_size=kw, stride=2, padding=padw), use_spectral_norm),
        ...
        layers += [spectral_norm_if(nn.Conv2d(ndf * nf_mult, 1, kernel_size=kw, stride=1, padding=padw), use_spectral_norm)]
