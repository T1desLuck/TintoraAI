...
class ObjectMemoryModule(nn.Module):
    ...
    def regional_pool(self, x: torch.Tensor, target_dim: Optional[int] = None) -> Tuple[torch.Tensor, Tuple[int, int]]:
        """
        ...
        if target_dim is None: # L2-нормализация только для признаков, не для цветовых значений
            regions = l2n(regions)  # нормализация по регионам
        return regions, (g, g)

    def forward(self, F2: torch.Tensor, gt_ab: Optional[torch.Tensor] = None, read_only: Optional[bool] = None):
        """
        ...
        if gt_ab is not None and not read_only:
            ...
            gt_ab_pooled = F.adaptive_avg_pool2d(gt_ab, output_size=grid_hw) # (B,2,g,g)
    ...
    @torch.no_grad()
    def ema_update(self, regions: torch.Tensor, attn: torch.Tensor, topi: Optional[torch.Tensor], gt_ab_regions: Optional[torch.Tensor]):
        """
        ...
        if topi is not None:
            ...
            flat_feat = regions.unsqueeze(2).expand(B, R, k, D).reshape(-1, D)  # (B*R*k,D)
            ...
            if gt_ab_regions is not None:
                ...
                delta_mu = torch.zeros(self.P, 2, device=delta.device, dtype=flat_color.dtype)
                delta_sigma = torch.zeros_like(delta_mu)
                delta_mu.index_add_(0, flat_idx, flat_w.unsqueeze(-1) * flat_color)
                delta_sigma.index_add_(0, flat_idx, flat_w.unsqueeze(-1) * (flat_color ** 2))
        else:
            # случай полного внимания
            # delta = sum_{b,r} attn[b,r,p] * regions[b,r]
            B, R, P = attn.shape
            delta = attn.reshape(-1, P).t() @ regions.reshape(-1, regions.shape[-1])  # (P,D)
            sup = attn.sum(dim=(0, 1))  # (P,)

            # Обновление цветовой статистики, если доступна
            if gt_ab_regions is not None:
                # delta_mu = sum_{b,r} attn[b,r,p] * gt_ab_regions[b,r]
                delta_mu = attn.reshape(-1, P).t() @ gt_ab_regions.reshape(-1, 2) # (P,2)
                # delta_sigma = sum_{b,r} attn[b,r,p] * (gt_ab_regions[b,r]**2)
                delta_sigma = attn.reshape(-1, P).t() @ (gt_ab_regions.reshape(-1, 2)**2) # (P,2)

        # избегаем деления на ноль
        mask = sup > 0
        updated = self.prototypes.clone()
        updated[mask] = l2n(m * updated[mask] + (1 - m) * (delta[mask] / sup[mask].unsqueeze(-1)))
        ...
        if gt_ab_regions is not None:
            ...
            new_sigma = m * self.prot_sigma_ab[mask] + (1-m) * (delta_sigma[mask] / sup_masked)
