...
try:  # PIL >= 9
    ...
    PILResampling = _PILResampling
except Exception:  # fallback for older PIL
    PILResampling = types.SimpleNamespace(
        NEAREST=0,
        BILINEAR=2,
        BICUBIC=3,
        LANCZOS=1,
...
def _resolve_resample(filter_name: str | int | None) -> int:
    ...
    # Совместимость: используем PIL.Image.Resampling (или фолбэк) вместо атрибутов Image.*
    ...
    if name in ("bilinear", "linear"):
        return PILResampling.BILINEAR
    if name in ("nearest", "nn"):
        return PILResampling.NEAREST
    # fallback
    return PILResampling.LANCZOS
...
def resize_shorter_side_and_center_crop(
    img: Image.Image, size: int, resample: str | int | None = None
) -> Image.Image:
    """
...
def resize_shorter_side_then_random_crop(
    img: Image.Image, size: int, resample: str | int | None = None
) -> Image.Image:
    """
...
def defect_small_spots(
    Ln: np.ndarray,
    p: float = 0.3,
    max_spots: int = 3,
    size_range: Tuple[int, int] = (2, 10),
    strength: float = 0.2,
) -> np.ndarray:
    """
    ...
    for _ in range(n):
        ...
        out[y : y + sz, x : x + sz, 0] = np.clip(
            out[y : y + sz, x : x + sz, 0] + delta, -1.0, 1.0
        )
    return out
...
def augment_L_defects(Ln: np.ndarray, cfg: dict | None) -> np.ndarray:
    ...
    # Blur
    out = defect_blur(
        out, p=float(cfg.get("blur_p", 0.0)), radius=float(cfg.get("blur_radius", 0.8))
    )
    return out
