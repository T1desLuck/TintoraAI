...
from .utils.metrics import ssim, try_dists, ciede2000
from .utils import load_config
from .utils import preload_dists
...
def main():
    ...
    parser.add_argument(
        "--no-resume",
        action="store_true",
        help="Не возобновлять обучение даже при наличии latest чекпоинта",
    )
    ...
    if torch.cuda.is_available():
        ...
        if device.type == "cuda" and device.index is None:
            device = torch.device(
                f"cuda:{local_rank}" if torch.cuda.device_count() > 1 else "cuda:0"
            )
    ...
    writer = (
        ...
        if (cfg.get("logging", {}).get("tensorboard", True) and is_main_process())
        else None
    )
    ...
    train_dir = paths_cfg.get(
        "train_dir", os.path.join(data_root, data_cfg.get("train_dir", "train"))
    )
    val_dir = paths_cfg.get(
        "val_dir", os.path.join(data_root, data_cfg.get("val_dir", "val"))
    )
    ...
    if len(ds) == 0:
        print(
            f"Изображения не найдены в {train_dir}. Добавьте изображения, чтобы начать обучение."
        )
        return
    sampler = None
    if ddp_enabled:
    ...
    # Предзагрузка DISTS (если включён) — до цикла обучения
    if do_val and bool(val_cfg.get("dists", False)):
        try:
            preload_dists()
        except Exception:
            pass
    ...
    if ddp_enabled:
        model = DDP(
            ...
            find_unused_parameters=runtime.get("ddp", {}).get(
    ...
    class ModelEMA:
        ...
        def update(self, model, decay: float):
            ...
            for (n_s, p_s), (n, p) in zip(
                self.shadow.state_dict().items(), src.state_dict().items()
            ):
    ...
    # GAN loss with optional label smoothing (for BCE)
    if use_gan:
        loss_type = str(gan_cfg.get("loss_type", "hinge"))
        if loss_type == "bce":
            smooth = float(gan_cfg.get("label_smooth", 0.0))
            real_label = max(0.0, 1.0 - smooth)
            fake_label = min(1.0, 0.0 + smooth)
            loss_gan = GANLoss(
                loss_type=loss_type, real_label=real_label, fake_label=fake_label
            )
        else:
            loss_gan = GANLoss(loss_type=loss_type)
    else:
        loss_gan = None
    ...
    # Подготовка имён чекпоинтов (используются как для резюма, так и для сохранения)
    ...
    latest_names = ckpt_cfg.get(
        "latest_names", {"model": "latest.pth", "ema": "latest_ema.pth"}
    )
    best_names = ckpt_cfg.get(
        "best_names", {"model": "best.pth", "ema": "best_ema.pth"}
    )

    # Инициализация счётчиков до попытки резюма
    best_score = None
    start_epoch = int(train_cfg.get("start_epoch", 1))
    # Глобальный счётчик шагов (для TensorBoard)
    global_step = 0

    # Попытка авто-резюма: если latest.pth существует и не указан --no-resume
    ckpt_dir = Path(paths.get("checkpoints", "checkpoints"))
    ckpt_dir.mkdir(parents=True, exist_ok=True)
    latest_model_path = ckpt_dir / latest_names.get("model", "latest.pth")
    latest_ema_path = ckpt_dir / latest_names.get("ema", "latest_ema.pth")
    if (not args.no_resume) and latest_model_path.exists():
        try:
            state = torch.load(latest_model_path, map_location=device)
            model_to_load = model.module if isinstance(model, DDP) else model
            sd = state.get("model", state)
            model_to_load.load_state_dict(sd, strict=False)
            # Опционально восстановим оптимизатор/глобальные переменные
            if "opt" in state:
                try:
                    opt.load_state_dict(state["opt"])
                except Exception:
                    pass
            if use_gan and optD is not None and ("optD" in state):
                try:
                    optD.load_state_dict(state["optD"])
                except Exception:
                    pass
            if "epoch" in state:
                # Продолжим со следующей эпохи
                start_epoch = max(int(state.get("epoch", start_epoch)), start_epoch) + 1
            if "global_step" in state:
                global_step = int(state.get("global_step", global_step))
            if "best_score" in state:
                best_score = state.get("best_score", best_score)
            # EMA восстанавливаем из отдельного файла latest_ema.pth (если есть)
            if use_ema and ema is not None and latest_ema_path.exists():
                try:
                    ema_state = torch.load(latest_ema_path, map_location=device)
                    if "model" in ema_state:
                        ema.shadow.load_state_dict(
                            ema_state["model"]
                        )  # shadow имеет state_dict модели
                except Exception:
                    pass
            if is_main_process():
                print(
                    f"[RESUME] Обнаружен чекпоинт: {latest_model_path}. Продолжаю с эпохи {start_epoch} (global_step={global_step})."
                )
        except Exception as e:
            if is_main_process():
                print(
                    f"[RESUME] Не удалось загрузить latest чекпоинт ({latest_model_path}): {e}. Начинаю заново."
                )
    # Куррикулум-расписание
    phase = int(train_cfg.get("curriculum_phase", 0))
    cur_cfg = (
        train_cfg.get("curriculum", {})
        if train_cfg.get("curriculum", {}).get("enabled", False)
        else None
    )
    ...
    dlb_strategy = str(dlb_cfg.get("strategy", "ema")).lower()
    dynamic_balancer = (
        DynamicLossBalancer(decay=dlb_decay, strategy=dlb_strategy) if use_dlb else None
    )
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        for i, (L, ab, *_) in enumerate(pbar):
            ...
            with torch.cuda.amp.autocast(enabled=runtime.get("amp", True)):
                ...
                if ssl_enabled and phase <= -1:
                    ...
                    F3_up = torch.nn.functional.interpolate(
                        F3, size=(h2, w2), mode="bilinear", align_corners=False
                    )
                    ...
                    if phase >= 3 and lam_cluster > 0.0:
                        ...
                        mem_map = out.get(
                            "mem_map",
                            torch.zeros(
                                L.size(0), 256, L.size(2), L.size(3), device=L.device
                            ),
                        )
            ...
            if adv_active:
                ...
                with torch.cuda.amp.autocast(enabled=runtime.get("amp", True)):
                    ...
                    loss_D = 0.5 * (
                        loss_gan(pred_real, True, for_discriminator=True)
                        + loss_gan(pred_fake, False, for_discriminator=True)
                    )
                    if do_r1:
                        # R1 penalty: (gamma/2) * E[||grad D(x_real)||^2]
            ...
            if not is_ssl_phase:
                if use_dlb and dynamic_balancer is not None:
                    # Формируем контекст неопределённости для entropy-aware стратегии
                    ctx = None
                    if dlb_strategy == "entropy_aware" and l_entropy is not None:
                        try:
                            ctx = {"entropy": float(l_entropy.detach().item())}
                        except Exception:
                            ctx = {"entropy": 0.0}
                    lams = dynamic_balancer.compute_weights(base_lams, context=ctx)
            ...
            if writer is not None:
                ...
                if l_entropy is not None:
                    writer.add_scalar(
                        "train/loss_entropy", l_entropy.item(), global_step
                    )
                if l_ds is not None:
                    writer.add_scalar("train/loss_ds", l_ds.item(), global_step)
                if l_cluster is not None:
                    writer.add_scalar(
                        "train/loss_cluster", l_cluster.item(), global_step
                    )
                if use_dlb and dynamic_balancer is not None:
                    ...
                    writer.add_scalar(
                        "train/dlb_w_entropy", lams["entropy"], global_step
                    )
                    writer.add_scalar("train/dlb_w_ds", lams["ds"], global_step)
                    writer.add_scalar(
                        "train/dlb_w_cluster", lams["cluster"], global_step
                    )
                ...
                if adv_active:
                    ...
                    writer.add_scalar(
                        "train/loss_adv_G",
                        l_adv_term.item() if l_adv_term is not None else 0.0,
                        global_step,
                    )
        ...
        if is_main_process():
            ...
            active_losses = " ".join(
                [f"{k}={v:.1f}" for k, v in loss_weights.items() if v > 0]
            )
        ...
        if do_val and (epoch % int(train_cfg.get("validate_every_epochs", 1)) == 0):
            model.eval()
            ssim_vals = []
            dists_wrap = try_dists() if bool(val_cfg.get("dists", False)) else None
            dists_vals = []
            use_ciede = bool(val_cfg.get("ciede2000", False))
            ciede_vals = []
            with torch.no_grad():
                for L, ab, _ in dl_val:
                    ...
                    if dists_wrap is not None and dists_wrap.enabled:
                        val = dists_wrap(rgb_pred, rgb_gt)
                        if val is not None:
                            dists_vals.append(val.item())
                    if use_ciede:
                        # Собираем Lab в абсолютных единицах: L in [0,100], a/b в исходных диапазонах
                        L_abs = torch.clamp((L + 1.0) * 50.0, 0.0, 100.0)
                        a_pred = torch.clamp(out["a"], -128.0, 127.0)
                        b_pred = torch.clamp(out["b"], -128.0, 127.0)
                        a_gt = torch.clamp(ab[:, :1], -128.0, 127.0)
                        b_gt = torch.clamp(ab[:, 1:2], -128.0, 127.0)
                        lab_pred = torch.cat([L_abs, a_pred, b_pred], dim=1)
                        lab_gt = torch.cat([L_abs, a_gt, b_gt], dim=1)
                        ciede_vals.append(ciede2000(lab_pred, lab_gt).item())
            ssim_mean = float(sum(ssim_vals) / max(1, len(ssim_vals)))
            if writer is not None:
                writer.add_scalar("val/ssim", ssim_mean, epoch)
            if dists_vals and writer is not None:
                writer.add_scalar(
                    "val/dists", float(sum(dists_vals) / len(dists_vals)), epoch
                )
            if ciede_vals and writer is not None:
                writer.add_scalar(
                    "val/ciede2000", float(sum(ciede_vals) / len(ciede_vals)), epoch
                )
            ...
            if is_main_process():
                ...
                if save_latest:
                    ...
                    if use_gan and optD is not None:
                        latest_payload["optD"] = optD.state_dict()
                    torch.save(
                        latest_payload,
                        ckpt_dir / latest_names.get("model", "latest.pth"),
                    )
                    if use_ema and ema is not None:
                        torch.save(
                            {"model": ema.state_dict(), "epoch": epoch},
                            ckpt_dir / latest_names.get("ema", "latest_ema.pth"),
                        )
                ...
                if save_best:
                    ...
                    elif best_metric == "dists":
                        if dists_vals:
                            score = float(sum(dists_vals) / len(dists_vals))
                            is_better = (best_score is None) or (score < best_score)
                        else:
                            is_better = False
                            score = None
                    elif best_metric == "ciede2000":
                        if ciede_vals:
                            score = float(sum(ciede_vals) / len(ciede_vals))
                            is_better = (best_score is None) or (score < best_score)
                        else:
                            is_better = False
                            score = None
                    ...
                    if is_better:
                        ...
                        torch.save(
                            ...
                            ckpt_dir / best_names.get("model", "best.pth"),
                        )
                        if use_ema and ema is not None:
                            torch.save(
                                {
                                ...
                                ckpt_dir / best_names.get("ema", "best_ema.pth"),
                            )
    ...
    if is_main_process() and not do_val:
        ...
        if save_latest:
            ...
            if use_gan and optD is not None:
                latest_payload["optD"] = optD.state_dict()
            torch.save(
                latest_payload, ckpt_dir / latest_names.get("model", "latest.pth")
            )
            if use_ema and ema is not None:
                torch.save(
                    {"model": ema.state_dict(), "epoch": epoch},
